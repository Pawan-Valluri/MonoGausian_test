...
from utils.viewer import Mini3DViewer
...
class TestRunner():
    ...
    def run(self):
        ...
        for img_index in range(len(self.plot_dataloader)):
            ...
            img_names = model_input['img_name'][:, 0].clone().cpu().numpy()
    ...
    def run_showcase_cv2(self):
        # cfg = tyro.cli(Config)
        viewer = Mini3DViewer(1280, 720)
        cv2.namedWindow("3D Viewer")
        cv2.setMouseCallback("3D Viewer", viewer.on_mouse)
...
>>>>>>> ecdf3c5 (cv2 version of gui (ft. chatgpt))
                ...
                #     viewer.camera.camera_center,
                #     viewer.camera.tanfovx,
                #     viewer.camera.tanfovy,
                #     model_outputs["bg_color"],
                #     model_outputs["image_h"],
                #     model_outputs["image_w"],
                #     model_outputs["xyz"],
