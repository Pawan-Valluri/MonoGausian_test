...
class MonogaussianAvatar(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.FLAMEServer = FLAME(
            "./flame/FLAME2020/generic_model.pkl",
        ...
        (
            self.FLAMEServer.canonical_verts,
            self.FLAMEServer.canonical_pose_feature,
            self.FLAMEServer.canonical_transformations,
        ) = self.FLAMEServer(
            expression_params=self.FLAMEServer.canonical_exp,
        ...
        self.deformer_network = ForwardDeformer(
            FLAMEServer=self.FLAMEServer, **conf.get_config("deformer_network")
        )
        self.rendering_network = RenderingNetwork(
            **conf.get_config("rendering_network")
        )
        self.gaussian_deformer_network = GaussianNetwork(
            **conf.get_config("gaussian_network")
        )
        self.ghostbone = self.deformer_network.ghostbone
        if self.ghostbone:
            self.FLAMEServer.canonical_transformations = torch.cat(
                [
                    torch.eye(4).unsqueeze(0).unsqueeze(0).float().cuda(),
                    self.FLAMEServer.canonical_transformations,
                ],
                1,
            )
        ...
        self.return_render_info = False

    def _compute_canonical_normals_and_feature_vectors(self):
        geometry_output, scales, rotations, opacity = self.geometry_network(
            self.pc.points.detach()
        )
    ...
    def _render(
        ...
        screenspace_points = (
            torch.zeros_like(xyz, dtype=xyz.dtype, requires_grad=True, device="cuda")
            + 0
        )
        try:
    ...
    def forward(self, input):
        ...
        verts, pose_feature, transformations = self.FLAMEServer(
            expression_params=expression, full_pose=flame_pose
        )
        ...
        transformed_points, rgb_points, scale_vals, rotation_vals, opacity_vals = (
            self.get_rbg_value_functorch(
                pnts_c=self.pc.points,
                feature_vectors=feature_vector,
                pose_feature=pose_feature.unsqueeze(1)
                .expand(-1, n_points, -1)
                .reshape(total_points, -1),
                betas=expression.unsqueeze(1)
                .expand(-1, n_points, -1)
                .reshape(total_points, -1),
                transformations=transformations.unsqueeze(1)
                .expand(-1, n_points, -1, -1, -1)
                .reshape(total_points, *transformations.shape[1:]),
            )
        )

        shapedirs, posedirs, lbs_weights, pnts_c_flame = (
            self.deformer_network.query_weights(self.pc.points.detach())
        )
        ...
        offset_scale, offset_rotation, offset_opacity, offset_color = (
            self.gaussian_deformer_network(offset)
        )
        ...
        flame_distance, index_batch, _ = knn_points(
            pnts_c_flame.unsqueeze(0), knn_v, K=1, return_nn=True
        )
    ...
    def forward_showcase(self, input, idx=0):
        self._output = {}
        ...
        # idx is in input(this is to select which of the input in batch to be rendered)
        world_view_transform_i = world_view_transform[idx]
        full_proj_transform_i = full_proj_transform[idx]
        camera_center_i = camera_center[idx]
        tanfovx_i = tanfovx[idx]
    ...
    def get_rbg_value_functorch(
        self, pnts_c, feature_vectors, pose_feature, betas, transformations
    ):
        if pnts_c.shape[0] == 0:
        ...
        def _func(pnts_c, betas, transformations, pose_feature):
            ...
            shapedirs, posedirs, lbs_weights, pnts_c_flame = (
                self.deformer_network.query_weights(pnts_c)
            )
            ...
            pnts_d = self.FLAMEServer.forward_pts(
                pnts_c_flame,
                betas,
                transformations,
                pose_feature,
        ...
        transformations = transformations.reshape(
            batch_size, n_points, *transformations.shape[1:]
        ).transpose(0, 1)
        pose_feature = pose_feature.reshape(
            batch_size, n_points, *pose_feature.shape[1:]
        ).transpose(0, 1)
        grads_batch, pnts_d = vmap(
            jacfwd(_func, argnums=0, has_aux=True), out_dims=(0, 0)
        )(pnts_c, betas, transformations, pose_feature)

        pnts_d = pnts_d.reshape(-1, batch_size, 3).transpose(0, 1).reshape(-1, 3)
        feature_vectors = (
            feature_vectors.unsqueeze(0)
